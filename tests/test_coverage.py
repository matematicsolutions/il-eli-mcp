...
from il_eli_mcp.server import il_coverage


def _coverage():
    fn = getattr(il_coverage, "fn", il_coverage)
...
def test_tool_writes_an_audit_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("IL_ELI_AUDIT_DIR", str(tmp_path))
    _coverage()
    lines = [
        line
    ...
    assert any("il_coverage" in line for line in lines), (
        "an audit entry was written but it does not name il_coverage"
    )
